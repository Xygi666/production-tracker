...
class App {
  ...
  cacheDOM(){
    ...
    // Guard logs
    [
      'settingsBtn','exportJsonBtn','reloadBtn','productSearch','quantityInput','addRecordBtn',
      'recordsList','statsGrid','historyList','settingsModal','themeSelect'
    ].forEach(k=>{ if(!this[k]) console.warn('[DOM missing]',k); });
  }
  ...
  bindEvents(){
    ...
    // Header
    this.settingsBtn && this.settingsBtn.addEventListener('click',()=>this.openSettings());
    this.exportJsonBtn && this.exportJsonBtn.addEventListener('click',()=>this.exportJson());
    this.reloadBtn && this.reloadBtn.addEventListener('click',()=>location.reload());

    // Search
    this.productSearch && this.productSearch.addEventListener('input',()=>this.updateProductSuggestions());
    this.productSearch && this.productSearch.addEventListener('focus',()=>this.updateProductSuggestions());
    this.clearSearchBtn && this.clearSearchBtn.addEventListener('click',()=>this.clearSearch());
    document.addEventListener('click',e=>{
      if(this.productSuggestions && !this.productSuggestions.contains(e.target)
         && e.target!==this.productSearch && e.target!==this.clearSearchBtn){
    ...
    this.decreaseBtn && this.decreaseBtn.addEventListener('click',()=>{
    ...
    this.increaseBtn && this.increaseBtn.addEventListener('click',()=>{
      const c=Math.max(1,Math.floor(parseFloat(this.quantityInput.value)||1));
      this.quantityInput.value=c+1; this.calculateSum();
    });

    this.addRecordBtn && this.addRecordBtn.addEventListener('click',()=>this.addRecord());
    this.exportCsvBtn && this.exportCsvBtn.addEventListener('click',()=>this.exportCsv());

    // Manual shift
    this.addManualShiftBtn && this.addManualShiftBtn.addEventListener('click',()=>this.addManualShift());

    // History
    this.filterDate && this.filterDate.addEventListener('change',()=>this.renderHistory());
    this.filterAction && this.filterAction.addEventListener('change',()=>this.renderHistory());
    this.exportHistoryBtn && this.exportHistoryBtn.addEventListener('click',()=>this.exportHistory());

    // Settings
    this.closeSettingsBtn && this.closeSettingsBtn.addEventListener('click',()=>this.closeSettings());
    this.cancelSettingsBtn && this.cancelSettingsBtn.addEventListener('click',()=>this.closeSettings());
    this.saveSettingsBtn && this.saveSettingsBtn.addEventListener('click',()=>this.saveSettings());
    this.settingsTabs.forEach(t=>t.addEventListener('click',e=>this.switchSettingsPanel(e.currentTarget.dataset.tab)));
    this.settingsModal && this.settingsModal.addEventListener('click',e=>{
      if(e.target===this.settingsModal || e.target.classList.contains('modal__backdrop')) this.closeSettings();
    });

    // Theme change
    this.themeSelect && this.themeSelect.addEventListener('change',()=>{
    ...
    // Products
    this.addProductBtn && this.addProductBtn.addEventListener('click',()=>this.openProductModal());
    this.importProductsBtn && this.importProductsBtn.addEventListener('click',()=>this.importProductsFile.click());
    this.importProductsFile && this.importProductsFile.addEventListener('change',e=>this.importProducts(e.target.files[0]));

    // Product modal
    this.closeProductBtn && this.closeProductBtn.addEventListener('click',()=>this.closeProductModal());
    this.cancelProductBtn && this.cancelProductBtn.addEventListener('click',()=>this.closeProductModal());
    this.saveProductBtn && this.saveProductBtn.addEventListener('click',()=>this.saveProduct());
    this.productModal && this.productModal.addEventListener('click',e=>{
      if(e.target===this.productModal || e.target.classList.contains('modal__backdrop')) this.closeProductModal();
    });

    // Backup
    this.emailBackupBtn && this.emailBackupBtn.addEventListener('click',()=>this.shareBackup('email'));
  }
  ...
  clearSearch(){
    ...
    this.productSearch.value=''; this.selectedProductId=null;
    this.updateProductSuggestions(); this.calculateSum(); this.updateClearButton();
  }
  updateClearButton(){
    if(!this.clearSearchBtn || !this.productSearch) return;
    this.productSearch.value.trim() ? this.clearSearchBtn.classList.remove('hidden')
  ...
  renderSuggestions(items){
    if(!this.productSuggestions) return;
    if(!items.length){ this.productSuggestions.innerHTML=''; this.productSuggestions.classList.add('hidden'); this.selectedProductId=null; this.calculateSum(); return; }
    this.productSuggestions.classList.remove('hidden');
    this.productSuggestions.innerHTML='';
    items.forEach(p=>{
      const row=document.createElement('div');
      row.className='suggestion-item';
      row.innerHTML=`<span class="suggestion-star ${p.favorite?'suggestion-star--favorite':''}" data-id="${p.id}">★</span>
      ...
      row.querySelector('.suggestion-text').addEventListener('click',()=>{
        ...
        this.selectedProductId=p.id; this.hideSuggestions(); this.calculateSum(); this.updateClearButton();
      });
      row.querySelector('.suggestion-star').addEventListener('click',(e)=>{ e.stopPropagation(); this.toggleFavorite(p.id); });
  ...
  toggleFavorite(productId){
    const p=(this.data.products||[]).find(x=>x.id===productId); if(!p) return;
    p.favorite=!p.favorite; this.save(); this.updateProductSuggestions(); this.renderProductsList();
  ...
  renderPresets(){
    ...
    (this.data.presets||[]).forEach(v=>{
      ...
      b.addEventListener('click',()=>{
        ...
        if(this.quantityInput){ this.quantityInput.value = base===0 ? Number(v) : base+Number(v); }
        this.calculateSum();
      });
  ...
  currentProduct(){
    if(this.selectedProductId) return (this.data.products||[]).find(p=>p.id===this.selectedProductId)||null;
    const q=(this.productSearch?.value||'').trim().toLowerCase(); if(!q) return null;
  ...
  calculateSum(){
    ...
    if(this.sumAmount) this.sumAmount.textContent = p? `${(qty*p.price).toFixed(2)} ${CONFIG.DEFAULT_CURRENCY}` : `0 ${CONFIG.DEFAULT_CURRENCY}`;
  }
  ...
  addRecord(){
    ...
    this.log('add_record',rec,'Добавлена запись'); this.save();
    if(this.quantityInput) this.quantityInput.value=''; this.calculateSum(); this.renderRecords(); this.renderStatistics();
  }
  ...
  deleteRecord(id){
    ...
    if(i>=0){ const old=this.data.entries[i]; this.data.entries.splice(i,1); this.log('delete_record',old,'Удалена запись'); this.save(); this.renderRecords(); this.renderStatistics(); }
  }
  ...
  addManualShift(){
    ...
    (this.data.shifts=this.data.shifts||[]).push(sh); this.log('add_shift',sh,`Добавлена смена: ${h} ч`); this.save(); this.applyManualDefaultHours(); this.renderStatistics();
  }

  renderStatistics(){
    if(!this.statsGrid) return;
    const now=new Date(); const y=now.getFullYear(); const m=now.getMonth()+1;
    const month=(this.data.entries||[]).filter(e=>{const d=new Date(e.date);return d.getFullYear()===y&&(d.getMonth()+1)===m;});
    ...
    let auto=0; if(this.data.salary.workSchedule!=='off' && typeof WorkScheduleManager==='function'){ const ws=new WorkScheduleManager(); ws.updateSettings(this.data.salary); auto=ws.calculateAutoHours(y,m); }
  ...
  openSettings(){
    ...
    if(this.workSchedule) this.workSchedule.value=s.workSchedule??'off';
    if(this.hoursPerShift) this.hoursPerShift.value=s.hoursPerShift??12;
    if(this.scheduleStartDate) this.scheduleStartDate.value=s.scheduleStartDate??'';
    if(this.themeSelect) this.themeSelect.value=this.data.theme||'classic';
    if(this.presetsInput) this.presetsInput.value=(this.data.presets||[]).join(',');
  ...
  saveSettings(){
    this.data.salary={
      ...
      workSchedule:this.workSchedule?.value||'off',
    ...
    if(theme!==this.data.theme){ this.data.theme=theme; this.applyTheme(theme); Safe.sr(CONFIG.STORAGE_KEYS.THEME, theme); }
  ...
  openProductModal(productId=null){
    ...
    if(productId){
      ...
      if(p){ this.productModalTitle.textContent='Изменить продукт'; this.productNameInput.value=p.name; this.productPriceInput.value=p.price; }
    } else {
      this.productModalTitle.textContent='Добавить продукт'; this.productNameInput.value=''; this.productPriceInput.value='';
    }
  ...
  saveProduct(){
    ...
    if(this.editingProductId){
      const p=(this.data.products||[]).find(x=>x.id===this.editingProductId); if(p){ p.name=name; p.price=price; this.log('edit_product',p,'Изменён продукт'); }
    } else {
      const p={id:Date.now(),name,price,archived:false,created:new Date().toISOString(),favorite:false};
  ...
  renderProductsList(){
    ...
    (this.data.products||[]).forEach(p=>{
      ...
      row.innerHTML=`<div class="record-info"><div class="record-title">${star} ${this.esc(p.name)}</div><div class="record-details">${p.price}${CONFIG.DEFAULT_CURRENCY}</div></div>
  ...
  // History/Export/Save utils
  renderHistory(){ if(!this.historyList) return; const df=this.filterDate?.value; const af=this.filterAction?.value; let list=[...(this.data.log||[])]; if(df) list=list.filter(e=>new Date(e.timestamp).toISOString().slice(0,10)===df); if(af) list=list.filter(e=>e.action===af); list.sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)); this.historyList.innerHTML=list.length?'':'<div class="record-item"><div class="record-info"><div class="record-title">История пуста</div></div></div>'; list.forEach(e=>{ const d=new Date(e.timestamp); const row=document.createElement('div'); row.className='record-item'; row.innerHTML=`<div class="record-info"><div class="record-title">${this.esc(this.actionName(e.action))}</div><div class="record-details">${d.toLocaleDateString(CONFIG.DATE_FORMAT)} ${d.toLocaleTimeString(CONFIG.DATE_FORMAT,{hour:'2-digit',minute:'2-digit'})}</div><div class="record-details">${this.esc(e.details||'')}</div></div>`; this.historyList.appendChild(row); }); }
  ...
  download(content,name,type){ const blob=new Blob([content],{type}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=name; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url); }
  save(){ Safe.s(CONFIG.STORAGE_KEYS.PRODUCTS,this.data.products); Safe.s(CONFIG.STORAGE_KEYS.ENTRIES,this.data.entries); Safe.s(CONFIG.STORAGE_KEYS.SHIFTS,this.data.shifts); Safe.s(CONFIG.STORAGE_KEYS.SALARY,this.data.salary); Safe.s(CONFIG.STORAGE_KEYS.LOG,(this.data.log||[]).slice(-CONFIG.MAX_LOG)); Safe.s(CONFIG.STORAGE_KEYS.PRESETS,this.data.presets); Safe.sr(CONFIG.STORAGE_KEYS.THEME,this.data.theme||'classic'); }
  log(a,item,details){ (this.data.log=this.data.log||[]).push({id:Date.now()+Math.random()*1000|0,timestamp:new Date().toISOString(),action:a,item,details}); }
  ...
  importProducts(file){ if(!file) return; const reader=new FileReader(); reader.onload=e=>{ try{ const csv=e.target.result; const lines=csv.split('\n').filter(l=>l.trim()); if(lines.length<2) return alert('Файл должен содержать заголовки и данные'); const headers=lines[0].split(',').map(h=>h.trim().replace(/"/g,'')); const nameIdx=headers.findIndex(h=>h.toLowerCase().includes('назв')||h.toLowerCase().includes('name')); const priceIdx=headers.findIndex(h=>h.toLowerCase().includes('цен')||h.toLowerCase().includes('price')); if(nameIdx===-1||priceIdx===-1) return alert('Файл должен содержать колонки с названием и ценой'); let added=0; for(let i=1;i<lines.length;i++){ const cols=lines[i].split(',').map(c=>c.trim().replace(/"/g,'')); const name=cols[nameIdx]?.trim(); const price=parseFloat(cols[priceIdx]); if(name&&!isNaN(price)&&price>0){ const exists=(this.data.products||[]).some(p=>p.name.toLowerCase()===name.toLowerCase()); if(!exists){ (this.data.products=this.data.products||[]).push({id:Date.now()+Math.random(),name,price,archived:false,created:new Date().toISOString(),favorite:false}); added++; } } } this.save(); this.renderProductsList(); this.updateProductSuggestions(); alert(`Импортировано продуктов: ${added}`); }catch(err){ alert('Ошибка чтения файла'); } }; reader.readAsText(file); }
}
